...
## Fase 3A - Núcleo de Leitura Implementado

Na arquitetura atual, **o núcleo do servidor MCP de leitura foi implementado localmente (`mcp/`)**. A premissa absoluta é: **O MCP será SOMENTE LEITURA.** 
Os agentes de IA do portal não terão permissões de escrita (mutations) nas coleções. As ferramentas limitam-se a consultar repositórios em memória baseados nos contratos de Confiança (`TrustContract`).
