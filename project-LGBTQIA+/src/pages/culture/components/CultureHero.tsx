...
export default function CultureHero() {
  ...
  const { content } = useSiteSection('culture-hero', fallbackHero);
  
  const isPortuguese = (i18n.resolvedLanguage || 'pt-BR').startsWith('pt');
  const localizedContent = isPortuguese ? content : {
    ...content,
    title: content.title_en || content.title_es || t('culture.hero.title'),
    subtitle: content.subtitle_en || content.subtitle_es || t('culture.hero.eyebrow'),
    description: content.description_en || content.description_es || t('culture.hero.description'),
  };
  ...
  return (
    <section className="relative w-full bg-dark-700 pt-20 pb-10 md:pt-24 md:pb-20 px-4 md:px-6 lg:px-10 overflow-hidden">
      <div className="absolute inset-0 opacity-20">
        <img
          src={localizedContent.image || fallbackHero.image}
          alt=""
